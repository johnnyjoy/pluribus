...
func ToolDefinitions() []map[string]any {
	...
	return []map[string]any{
		{"name": "recall_context", "description": "Retrieve relevant prior knowledge from Pluribus. Use BEFORE complex reasoning, multi-step tasks, debugging, design decisions, or when uncertain. Helps avoid repeating mistakes and reuse proven patterns. WHAT: POST /v1/recall/compile with deterministic routing from your task text (task or task_description). Response includes mcp_context (why_now, bundle_counts) + recall_bundle. Same tool as memory_context_resolve — use whichever name your client lists." + layer1DefaultLoop + toolDoctrineHint, "inputSchema": schemaObj},
		{"name": "record_experience", "description": "Record a meaningful outcome into Pluribus. Use AFTER solving a problem, encountering a failure, discovering a pattern, or learning something reusable. Ensures future tasks benefit from this experience. WHAT: POST /v1/advisory-episodes; server qualifies inline — learning signals → immediate probationary memory (advisory applicability); weak text → reject bucket only. Optional auto-distill still creates pending candidates when enabled. Same tool as mcp_episode_ingest." + layer1DefaultLoop + toolDoctrineHint, "inputSchema": schemaObj},
