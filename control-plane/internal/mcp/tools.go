...
func ToolDefinitions() []map[string]any {
	...
	return []map[string]any{
		{"name": "recall_context", "description": "WHEN: Before planning, uncertain requirements, architecture, or multi-step work. WHY: Surfaces prior constraints, failures, patterns, and decisions so you do not repeat mistakes. WHAT: POST /v1/recall/compile with deterministic routing from your task text (task or task_description). Response includes mcp_context (why_now, bundle_counts) + recall_bundle. Same tool as memory_context_resolve — use whichever name your client lists." + layer1DefaultLoop + toolDoctrineHint, "inputSchema": schemaObj},
		{"name": "record_experience", "description": "WHEN: After incidents, non-trivial fixes, explicit decisions, or important discoveries. WHY: Writes an advisory episode so future tasks can reuse the outcome; optional server auto-distill → pending candidates. WHAT: POST /v1/advisory-episodes (summary required; optional tags, correlation_id, event_kind, entities). Not canonical memory until reviewed/promoted. Same tool as mcp_episode_ingest." + layer1DefaultLoop + toolDoctrineHint, "inputSchema": schemaObj},
