...
func GenerateMCPToolsMarkdown() string {
	...
	b.WriteString("Set **`PLURIBUS_TOOLS`** env or **`mcp.tools_tier`** in config (`core` | `standard` | `all`). Default **`core`** lists the loop tools only. **`tools/call`** still accepts all registered names regardless of tier.\n\n")
