...
var (
	...
	schemaRecordExperience = schemaObject(map[string]any{
		...
		"occurred_at":     propString("Optional RFC3339 when the experience happened (clock questions). Not parsed from English."),
	}, []string{"summary"})
...
func toolRegistry() []ToolSpec {
	recallDesc := "Use at the start of a substantive task, after context changes, or when uncertain whether prior constraints, decisions, failures, or patterns apply. Returns candidates (agent_grounding plus mcp_context); you must curate them into a short working context before acting. For clock/last-time questions pass RFC3339 occurred_after/occurred_before; the text then includes matching experiences. Full dump is in structuredContent — do not treat it as context. Mutates nothing. Part of the default agent loop (before complex work). Same handler as memory_context_resolve." + layer1DefaultLoop
	recordDesc := "Use after solving a problem, encountering a failure, discovering a reusable pattern, or confirming a decision. Records an experience (recallable) after meaningful outcomes. Write one dense claim, not an essay. Optional occurred_at (RFC3339). Pass used_memory_ids for memories you kept and used (server upvotes). Weak or padded text is rejected. Same handler as mcp_episode_ingest." + layer1DefaultLoop
	...
	return []ToolSpec{
		...
		{Name: "record_experience", Description: recordDesc, InputSchema: schemaRecordExperience, LoopRole: LoopPostOutcome, Risk: RiskMedium, Backend: "POST /v1/advisory-episodes", Output: "experience + formation status", Mutates: true, TestCoverage: "both", CallCoverage: CoverageUnitToolsCall, CallCoverageNote: "tool_call_coverage_test.go"},
		...
		{Name: "episode_search_similar", Description: "Search similar experiences (non-reject bucket). Prefer recall_context clock/last-time lane. Mutates nothing.", InputSchema: schemaEpisodeSimilar, LoopRole: LoopDiagnostic, Risk: RiskLow, Backend: "POST /v1/advisory-episodes/similar", Output: "similar experiences", Mutates: false, TestCoverage: "integration", CallCoverage: CoverageUnitToolsCall, CallCoverageNote: "tool_call_coverage_test.go"},
