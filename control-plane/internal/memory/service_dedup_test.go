...
func TestService_Create_duplicateBeforeInsert(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT id, kind, statement`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "kind", "statement", "statement_canonical", "statement_key", "authority", "applicability", "status", "deprecated_at", "ttl_seconds", "payload", "created_at", "updated_at"}).
			AddRow(existingID, api.MemoryKindDecision, stmt, memorynorm.StatementCanonical(stmt), sk, 5, "governing", "active", nil, nil, nil, time.Now(), time.Now()))
...
func TestService_Create_dedupDisabled_skipsLookup(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO memories`).
		WithArgs(sqlmock.AnyArg(), "decision", stmt, memorynorm.StatementCanonical(stmt), memorynorm.StatementKey(stmt), dedup, 0, "governing", "active", nil, nil).
		WillReturnRows(sqlmock.NewRows([]string{"id", "kind", "statement", "statement_canonical", "statement_key", "authority", "applicability", "status", "deprecated_at", "ttl_seconds", "payload", "created_at", "updated_at"}).
			AddRow(newID, api.MemoryKindDecision, stmt, memorynorm.StatementCanonical(stmt), memorynorm.StatementKey(stmt), 0, "governing", "active", nil, nil, nil, time.Now(), time.Now()))
