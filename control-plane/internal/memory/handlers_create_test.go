...
// Duplicate statement returns existing row without authority bump by default (Phase 7).
func TestHandlers_Create_duplicateMemory_noBlindReinforce(t *testing.T) {
	...
	if obj.ID != existingID || obj.Authority != 5 {
		t.Fatalf("got %+v", obj)
	}
...
func TestHandlers_Create_duplicateMemory_legacyReinforceWhenEnabled(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
