...
**Review queue (9.4):** Config **`require_review: true`** (or **`POST /v1/memory/promote`** with **`"require_review": true`**) creates promoted memories with **`status`** **`pending`** — an **explicit operator hold**, not the shipped formation default. With **`hive_defaults: true`**, new memory stays **`active` at capped authority**. **`pending`** rows remain in the recall pool with slightly lower weight; they are not a separate warehouse. **`POST /v1/memory/search`** defaults to **`status: active`** for admin listing; use **`status: pending`** to list review holds. **`PromoteResponse`** includes **`status`**; run-multi **`debug.promotion_decision`** includes **`gates.require_review`** and, on success, **`memory_status`**. **Decay:** existing **`ttl_seconds`** / **`deprecated_at`** on **`memories`** remain the operator surface for expiration.
