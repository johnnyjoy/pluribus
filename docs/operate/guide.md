...
This complements [get-started.md](../get-started.md) with **operations**: config, health, migrations, and CI.
...
| Shape | Command / notes |
|-------|-----------------|
| **Published image + Compose (public default)** | `docker compose -f docker-compose.install.yml` + `PLURIBUS_IMAGE=ghcr.io/<owner>/pluribus:<tag>` — [guides/container-install.md](../guides/container-install.md), [release/images.md](../release/images.md). |
| **Clone + build (dev)** | Repo root: `docker compose up -d` — builds API from `./control-plane`, **:8123**, migrations on boot. |
| **Postgres + Redis only** | `docker compose up -d postgres redis` — run `./pluribus` on host; **Boot** applies embedded SQL on startup. |
| **Local Postgres only** | [get-started.md](../get-started.md) § local binary. |
