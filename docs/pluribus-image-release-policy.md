...
The image contains **only the control-plane API** binary (`controlplane` + `entrypoint` + **`config.example.yaml`** as a template at **`/config/config.example.yaml`**). It does **not** ship a live runtime config. **PostgreSQL** and **Redis** are **not** baked in; use Compose (see [pluribus-container-install.md](pluribus-container-install.md)). Mount **`control-plane/configs/config.yaml`** (or your own file) at **`/config/config.yaml`** and set **`CONFIG=/config/config.yaml`**. For host development copy **`config.example.yaml`** → **`configs/config.local.yaml`** (see [control-plane/README.md](../control-plane/README.md)).
