...
| `record_experience` | `POST /v1/advisory-episodes` | Same row as **`mcp_episode_ingest`** — preferred name. Server runs **inline qualification**: learning signals → **probationary** `memories` + link; weak text → **`advisory_experiences`** reject bucket. Successful JSON may include **`memory_formation_status`**, **`probationary_memory_id`**, **`mcp_affordance`**. |
