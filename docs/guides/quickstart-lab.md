...
Starts Postgres, Redis, and **pluribus** on **`http://127.0.0.1:8123`**. The API waits for the DB, applies embedded baseline SQL, then serves.

---

## Alternative: published image only (no local build)

For a **registry** install (no `git clone` build), use **`docker-compose.install.yml`** — [container-install.md](container-install.md), [../release/images.md](../release/images.md). Set **`PLURIBUS_IMAGE`**, then:
...
| Operations | [operate/guide.md](../operate/guide.md), [get-started.md](../get-started.md) |
