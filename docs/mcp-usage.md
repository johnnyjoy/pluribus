...
After editing any MCP config file, **fully restart** the host application (Cursor, Claude Desktop, …) so it reloads servers.
...
1. **Ground** — `recall_get` or `recall_compile` **before** substantive work. Prefer **tags** + **retrieval_query** per [memory-doctrine.md](memory-doctrine.md).
...
4. **Learn** — after meaningful work, `curation_digest` → review → `curation_materialize` for governed promotion.
