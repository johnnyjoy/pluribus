...
| **YAML + integration runners** | Structured pass/fail per scenario id (not hand-wavy demos) | [`proof_scenarios_integration_test.go`](../control-plane/cm./pluribus/proof_scenarios_integration_test.go) |
...
- Operational semantic defaults and fallback: [operate/guide.md](operate/guide.md), [evaluation.md](evaluation.md), [pluribus-release-scope.md](scope.md).
...
RECALL_PROOF_RESULTS_OUT=/path/to/proof-scenario-results-latest.md \
  ...
  go test -tags=integration -count=1 ./control-plane/cm./pluribus -run TestIntegration_proofScenarioSuite
```
