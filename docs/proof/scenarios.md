...
Integration proof suite entrypoint: `TestIntegration_proofScenarioSuite` in [`cm./pluribus/proof_scenarios_integration_test.go`](../control-plane/cm./pluribus/proof_scenarios_integration_test.go). The API is wired via [`internal/apiserver`](../control-plane/internal/apiserver/router.go) (same as `cm./pluribus`).
...
RECALL_PROOF_RESULTS_OUT=/path/to/proof-scenario-results-latest.md \
  ...
  go test -tags=integration -count=1 ./cm./pluribus -run TestIntegration_proofScenarioSuite
```
...
**Index of all proof receipts:** [pluribus-proof-index.md](../proof/README.md).
