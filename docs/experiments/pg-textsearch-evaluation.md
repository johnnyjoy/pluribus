...
- **`make pg-textsearch-eval`** — automated seed, projection, BM25 reindex, query suite, and summary artifacts (go/no-go signal without manual SQL).
- **ETL/rebuild** via `make lexical-*` or `go run ./cmd/pg-textsearch-eval …`.
- **Experimental** HTTP `POST /v1/experimental/lexical/search` returns ranked `memory_id` when enabled.

## Go / no-go

Read **`docs/experiments/pg-textsearch-eval-latest.md`** (regenerated by `make pg-textsearch-eval`) and **`artifacts/pg-textsearch/eval.json`** for verify status, plausible-query ratio, and recommendation.
