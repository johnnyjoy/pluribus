...
| G10 | Docs coherence | **Sprawling** | ~101 docs; `operate/guide.md` says semantic is "on by default" (wrong); broken links to `memory-bank/` and missing files |
...
13. **Docs pass:** fix `operate/guide.md` semantic claim, dead `memory-bank/` links,
