> **ADVISORY — not canonical product truth.** See [README.md](README.md). Do not paste into integration packs.
...
- **REST integration (focused modules):** `cm./pluribus/advisory_episodes_episodic_integration_test.go`, `episode_distill_integration_test.go`, `advisory_auto_distill_integration_test.go` (auto vs explicit distill, merge, recall boundary); `internal/similarity/handlers_create_test.go` (episode **201** if auto-distill errors).
